...
(function() {
   ...
   * Directive: sticky
   */
  module.directive('sticky', ['$window', '$timeout', '$document', function($window, $timeout, $document) {
      return {
        ...
        link: function linkFn($scope, $elem, $attrs) {
          ...
          function stickElement(closestLine) {
            ...
            var cssElementFixedRightValue = $scope.absoluteToFixedCSSRight($elem);
           ...
           */
          var stickyBottomLineCalculate = function() {
            // Get Parent height, so we know when to bottom out for confined stickies
              ...
              stickyBottomLine = elementsDistanceFromScrollbarStart
                + elementsDistanceFromBottomOfParent
                - $elem[0].offsetHeight
                - marginBottom
          ...
          function getCSS($el, prop) {
            ...
            // ***The problem with the below code is it puts the display styling inline and will override
            // potential class values. i.e. a hidden element that was made visible will be set to the original
            // hidden value and ignore a potential class that made it visible
          ...
          $scope.getInitialDimensions = function() {
            ...
            return {
              ...
              cssRight: getCSS($elem, 'right'),
              width: cssElementWidth,
              // **The width of the element needs to be the internal width without padding.
              // **offsetWidth and clientWidth include padding and would keep making element wider
          ...
          $scope.absoluteToFixedCSSRight = function(elem) {
              ...
              var elemRight = $scope.pixelStringToInt($window.getComputedStyle($elem[0]).getPropertyValue('right'));
